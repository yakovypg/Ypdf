...
namespace YpdfLib.Imaging
{
    public static class ImageInserter
    {
        public static void ConvertToPdf(string destPath, IImagePageParameters pageParams, params string[] inputFiles)
        {
            ...
            if (pageParams.CompressionLevel != CompressionConstants.UNDEFINED_COMPRESSION)
                pdfWriter.SetCompressionLevel(pageParams.CompressionLevel);
            ...
            for (int i = 0; i < images.Length; ++i)
            {
                ...
                bool addAreaBreak = i < inputFiles.Length - 1;

                AddStylizedImageToDoc(doc, image, pageParams.HorizontalAlignment, addAreaBreak);
            }
        ...
        private static void AddStylizedImageToDoc(Document doc, Image image, HorizontalAlignment horizontalAlignment = HorizontalAlignment.CENTER, bool addAreaBreak = true)
        {
